...
The project goal is to make game recommendations for the customer. By comparing
the customer's collection of games to other collections I hope to find out where
the customer ranks within the gaming community and if there are any clear
preferences in game types or differences in preferences from the community. (All
data pulled from www.boardgamegeek.com)
...
ggplot(collection_size_tbl[1:800, ], aes(n)) +
  ...
  geom_label(aes(x=250, y=50), label="Customer's collection in teal")
```
...
This will be interesting later: Line drawing is next to the bottom of the list,
with 2 games.
...
mech_ratings_tbl <- arrange(calc_attr_ratings("boardgamemechanic"),
                            desc(n_cust_games))
ggplot(mech_ratings_tbl, aes(reorder(value, n_games), n_games)) +
  labs(title="Customer's Games per Mechanic", x="Game Mechanics", y=NULL) +
  geom_col(fill="blue") +
  coord_flip()
```
...
Mechanics at the top right are in games the customer enjoyed and has purchased
more often than other gamers, so they might be safe bets for games that 
will be liked.

```{r Plot ownership per mech, echo=FALSE, message=TRUE, warning=FALSE}
mrt <- subset(mech_ratings_tbl, cust_rating_wt >= 6)
ggplot(mrt, aes(cust_rating_wt, own_ratio, label=value)) +
...
mrt <- top_n(mech_ratings_tbl, topx, pref)
ggplot(mrt, aes(reorder(value, pref), pref)) +
  labs(title="Customer's Preferred Mechanics", x="Top Mechanics", 
       y="Rating by Customer - Rating by Others") +
  theme(legend.position="bottom") +
  geom_point() +
  geom_segment(aes(x=1, xend=topx, y=0, yend=0), 
               color="blue") +
  coord_flip()
```
...
gcat_ratings_tbl <- arrange(calc_attr_ratings("boardgamecategory"),
                            desc(n_cust_games))
ggplot(gcat_ratings_tbl[1:40, ], aes(reorder(value, n_games), n_games)) +
  labs(title="Games per Category", x="Top 40 Categories", y=NULL) +
...
grt <- subset(gcat_ratings_tbl, cust_rating_wt >= 6)
ggplot(grt, aes(cust_rating_wt, own_ratio, label=value)) +
...
grt <- top_n(gcat_ratings_tbl, topx, pref)
ggplot(grt, aes(reorder(value, pref), pref)) +
  labs(title="Customer's Preferred Categories", x="Top Categories", 
       y="Rating by Customer - Rating by Others") +
  theme(legend.position="bottom") +
  geom_point() +
  geom_segment(aes(x=1, xend=topx, y=0, yend=0),
